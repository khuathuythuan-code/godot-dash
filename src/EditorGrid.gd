...
@export_flags("x", "y") var symmetrize = int(Constants.AxisBitflag.NONE):
	set(value):
		symmetrize = value
		queue_redraw()

@export_flags("x", "y") var odd = int(Constants.AxisBitflag.NONE):
	set(value):
		odd = value
		queue_redraw()
...
func _draw() -> void:
	...
	for cell_x in grid_size.x + 1:
		...
		var _line_base_y = 0.0 if not symmetrize & Constants.AxisBitflag.Y else grid_size.y * cell_size.y
		...
		var cell_offset := 0.0
		if odd & Constants.AxisBitflag.X:
	...
	for cell_y in grid_size.y + 1:
		...
		var cell_offset := 0.0
		if odd & Constants.AxisBitflag.Y:
