...
func handle_place(block_palette_button_group: ButtonGroup, placed_objects_collider: Area2D, level: LevelProps) -> void:
	if get_viewport().gui_get_hovered_control() == editor_viewport and not edit_handler.gizmo:
		...
		if block_palette_ref != null and not texture_variation_overlapping(placed_objects_collider, block_palette_ref.type, block_palette_ref.id) \
				and (Input.is_action_pressed(&"editor_add", false) or Config.config.touch_screen and Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and !Editor.swipe):
			if pressed_button != null and (!Config.config.touch_screen or Editor.delete == false):
				# Create object
				var object: Node2D
		...
		elif (Input.is_action_pressed(&"editor_remove", false) or Config.config.touch_screen and Editor.delete) and placed_objects_collider.has_overlapping_areas():
			placed_object_rotation_degrees = 0.0
